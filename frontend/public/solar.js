...
function initAutocomplete() {
    ...
    console.log('Found autocomplete elements:', autoCompletes.length);

    // Convert HTMLCollection to array and iterate properly
    for (var i = 0; i < autoCompletes.length; i++) {
        var place = autoCompletes[i];
        console.log('Setting up autocomplete for:', place);

        place.onkeyup = function () {
            console.log('Keyup triggered on:', this);
            var f = this.form;
            if (f) {
                //if(this.value.length >= 2 && !this.getAttribute('gp_enabled')){
                if (!this.getAttribute('gp_enabled')) {
                    console.log('Initializing clickastro autocomplete');
                ...
                console.warn('No form found for input:', this);
            }
        }
    }
}
...
function fillInAddressMain(el) {
    ...
    if (!place || !place.address_components) {
        console.warn('Place is undefined or missing address_components for inputId:', el.inputId);
        return;
    }
